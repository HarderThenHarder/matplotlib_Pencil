...
class Pencil:
    ...
    def fill_circle(plt, O=[0, 0], R=1, color='c', alpha=1):
        ...
        for i in range(len(theta)):
            plt.plot([O[0], circle_point_x[i]], [O[1], circle_point_y[i]], color=color, alpha=alpha)

    @staticmethod
    def stroke_rect(plt, center=[0, 0], width=1, height=1, color='c', line_style='-', alpha=1):
        horn_x = [center[0] - width / 2, center[0] + width / 2, center[0] + width / 2, center[0] - width / 2, center[0] - width / 2]
        horn_y = [center[1] + height / 2, center[1] + height / 2, center[1] - height / 2, center[1] - height / 2, center[1] + height / 2]
    ...
    def fill_rect(plt, center=[0, 0], width=1, height=1, color='c', line_style='-', alpha=1):
        integrate_y = np.arange(center[1] - height / 2, center[1] + height / 2, 0.001)
